...
def fastANIFormat(contigs,outDir):
	...
	out_filename =  os.path.join(outDir,'fastANI_genomeList.txt')
	...
	assem = pd.read_csv(contigs)
	print(assem)
 
	new_DF = assem.drop(assem.columns[0],axis=1)
	new_DF.to_csv(out_filename , index = False, header = False)
